...
abstract contract ERC20WrapperUpgradeable is Initializable, ERC20Upgradeable {
    ...
    function __ERC20Wrapper_init_unchained(IERC20Upgradeable underlyingToken) internal onlyInitializing {
        ...
        ERC20WrapperStorage.layout()._underlying = underlyingToken;
    }
    ...
    function decimals() public view virtual override returns (uint8) {
        try IERC20MetadataUpgradeable(address(ERC20WrapperStorage.layout()._underlying)).decimals() returns (uint8 value) {
            return value;
        } catch {
    ...
    function underlying() public view returns (IERC20Upgradeable) {
        return ERC20WrapperStorage.layout()._underlying;
    }
    ...
    function depositFor(address account, uint256 amount) public virtual returns (bool) {
        ...
        SafeERC20Upgradeable.safeTransferFrom(ERC20WrapperStorage.layout()._underlying, sender, address(this), amount);
    ...
    function withdrawTo(address account, uint256 amount) public virtual returns (bool) {
        ...
        SafeERC20Upgradeable.safeTransfer(ERC20WrapperStorage.layout()._underlying, account, amount);
        return true;
    }
    ...
    function _recover(address account) internal virtual returns (uint256) {
        uint256 value = ERC20WrapperStorage.layout()._underlying.balanceOf(address(this)) - totalSupply();
        _mint(account, value);
        return value;
    }
}
