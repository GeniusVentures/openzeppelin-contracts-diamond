...
abstract contract VotesUpgradeable is Initializable, ContextUpgradeable, EIP712Upgradeable, IERC5805Upgradeable {
    ...
    function getPastVotes(address account, uint256 timepoint) public view virtual override returns (uint256) {
        ...
        return VotesStorage.layout()._delegateCheckpoints[account].upperLookupRecent(SafeCastUpgradeable.toUint32(timepoint));
    }
    ...
    function getPastTotalSupply(uint256 timepoint) public view virtual override returns (uint256) {
        require(timepoint < clock(), "Votes: future lookup");
        return VotesStorage.layout()._totalCheckpoints.upperLookupRecent(SafeCastUpgradeable.toUint32(timepoint));
    }
    ...
    function _transferVotingUnits(address from, address to, uint256 amount) internal virtual {
        if (from == address(0)) {
            _push(VotesStorage.layout()._totalCheckpoints, _add, SafeCastUpgradeable.toUint224(amount));
        }
        if (to == address(0)) {
            _push(VotesStorage.layout()._totalCheckpoints, _subtract, SafeCastUpgradeable.toUint224(amount));
        }
