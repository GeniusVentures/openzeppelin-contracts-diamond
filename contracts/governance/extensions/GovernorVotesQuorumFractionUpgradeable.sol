...
abstract contract GovernorVotesQuorumFractionUpgradeable is Initializable, GovernorVotesUpgradeable {
    ...
    function quorumNumerator(uint256 timepoint) public view virtual returns (uint256) {
        ...
        CheckpointsUpgradeable.Checkpoint224 memory latest = GovernorVotesQuorumFractionStorage.layout()._quorumNumeratorHistory._checkpoints[length - 1];
        if (latest._key <= timepoint) {
            return latest._value;
        }

        // Otherwise, do the binary search
        return GovernorVotesQuorumFractionStorage.layout()._quorumNumeratorHistory.upperLookupRecent(SafeCastUpgradeable.toUint32(timepoint));
    }
    ...
    function quorum(uint256 timepoint) public view virtual override returns (uint256) {
        return (GovernorVotesStorage.layout().token.getPastTotalSupply(timepoint) * quorumNumerator(timepoint)) / quorumDenominator();
    }
    ...
    function _updateQuorumNumerator(uint256 newQuorumNumerator) internal virtual {
        ...
        GovernorVotesQuorumFractionStorage.layout()._quorumNumeratorHistory.push(SafeCastUpgradeable.toUint32(clock()), SafeCastUpgradeable.toUint224(newQuorumNumerator));
